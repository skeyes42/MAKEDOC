...
**Note**
Generating a new canonical document requires the user to first supply line item information in the line item form. The context menu for the Tools option of Dashboard form contains:
- Line items
- Assembly
- Analytics
- Maintenance & Configuration

The Assembly option is grayed-out until the Line item information has been provided by the user in the Lineitem form. (See lineitems.md feature documentation). 

## Creating a document starting with canonical node list

1. System computes total price from the line item table. This determines the **tier** (low dollar amount, medium dollar amount, or high dollar amount)
2. User select the **phase** of procurement: requisition, solicitation or award.
3. From the the combination of tier and phase, system determines the **doc type**. This determines which clause library of appropriate for the procurement.
4. System collects all fill-in tags found in clauses selected from the clause library in the **transient clause list**.
5. System opens a modal **Fill-in form** to collect data for the fill-ins. This form shows fill-ins found in both the clause nodes and the header nodes.
6. User provides a title for the document in the Fill-in form.
7. User provides fill-in data and closes the Fill-in form.
8. For every clause in the transient clause list that has been edited, system writes a new user clause (UC-\<short guid\>) to the Node table. A new UC- row is always created, even when the source clause was itself a UC- clause — UC- rows are never updated in place.
9. System creates a new row in the **Instance table** from the transient node list and transient clause list (now referencing the newly written UC- node IDs).
10. System replaces the fill-in tags in the transient clauses with the data collected from the Fill-in form.
11. System assembles the document.
12. System opens the **file-save dialog**.
13. User saves the newly assembled document.
14. During document assembly, the system establishes a **section number count**. This integer is used by the system to fill-in the {SecNo} plain text fill-in. Starting from 1, the system fills in each {SecNo} tag, incrementing by 1 as each clause node in the assembly is encountered.
