...
- Once the system has built the new document, it automatically puts up the Line item form (pre-populated with line items of the selected document) modally to give the user an opportunity to view, change and save line item information. (Changing line item information is often done when creating an solicitation amendment document or a contract mod document.)
- If the user dismisses the line item form, then the line items for the source document carry through to the new built-from target document.
- Clicking Save upserts the new document's line item information in the LineItem table.

**Note**
If new or changed line items push the total into a different tier, the system continues to build the new document in the same tier as the source document and displays a message to that effect.
