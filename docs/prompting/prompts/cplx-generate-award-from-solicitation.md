# Generate a Award from a Solicitation for Complex Tier
...
## Phase 2 — Select Solicitation and Build to Award

3. In the MAKEDOC Dashboard document list, locate the most recent **Complex tier solicitation** row (Document type = "Complex tier solicitation", Tier = "Complex").
4. Right-click that row.
5. Choose **Build to award...** from the context menu.
6. In the **Select Document Type** dialog, choose **Complex tier award**.
7. Confirm the **MAKEDOC — Build To: Complex tier award** Document Assembly
   window opens. 

---

## Phase 3 — Generate the Award
...
11. In the Fill-in Values dialog, supply the following values
    ...
    - **Contract No.** = `555599999`
    - **Title** = `Award paper and boxes procurement`
...
18. Confirm the MAKEDOC Dashboard is now the active window. The new Micro tier
    award entry should appear in the list dated today.
...
**Dashboard entry for the new award is missing after save:**
