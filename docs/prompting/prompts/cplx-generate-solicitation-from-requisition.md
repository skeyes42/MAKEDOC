# Generate a Solicitation from a Requisition for Complex Tier
...
cycle, building from an existing complex-tier requisition.
...
3. In the MAKEDOC Dashboard document list, locate the most recent **Complex tier
   requisition** row (Document type = "Complex tier requisition", Tier = "Complex").
...
7. Confirm the **MAKEDOC — Build To: Complex tier solicitation** Document Assembly
   window opens. 
...
11. In the Fill-in Values dialog, supply the following values
    ...
    - **Solicitation Number** = `444446666`
    - **Title** = `Paper and boxes procurement`
...
16. Confirm the Document Assembly status bar reads
    "Saved: Complex tier solicitation_<filename>"
...
18. Confirm the MAKEDOC Dashboard is now the active window. The new Complex tier
