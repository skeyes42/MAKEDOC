# Generate a Solicitation from a Requisition for Micro Tier
...
3. In the MAKEDOC Dashboard document list, locate the most recent **Micro tier
   requisition** row (Document type = "Micro tier requisition", Tier = "micro").
4. Right-click that row.
5. Choose **Build to solicitation...** from the context menu.
6. In the **Select Document Type** dialog, choose **Micro tier solicitation**.
7. Confirm the **MAKEDOC — Build To: Micro tier solicitation** Document Assembly
   window opens. The clause list should show 6 clauses (NL-0014 through NL-0019).
...
11. In the Fill-in Values dialog, supply the following values
    ...
    - **Solicitation Number** = `987654321`
    - **Title** = `Solicit pencils and batteries`
...
14. The Windows **Save As** dialog opens. The default filename
    (e.g. `Micro tier solicitation_<uuid>_<datetime>.docx`) and default folder
    (`C:\temp\assembled_documents\`) are correct — do not change them.
15. Click **Save**.
16. Confirm the Document Assembly status bar reads
    "Saved: Micro tier solicitation_<filename> (instance ..., 6 nodes)."
