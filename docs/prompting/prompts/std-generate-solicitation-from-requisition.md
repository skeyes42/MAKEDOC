...
3. In the MAKEDOC Dashboard document list, locate the most recent **Standard tier
   requisition** row (Document type = "Standard tier requisition", Tier = "std").
4. Right-click that row.
5. Choose **Build to solicitation...** from the context menu.
6. In the **Select Document Type** dialog, choose **Standard tier solicitation**.
7. Confirm the **MAKEDOC — Build To: Standard tier solicitation** Document Assembly
   window opens. 
...
11. In the Fill-in Values dialog, supply the following values
    ...
    - **Solicitation Number** = `888899999`
    - **Title** = `Motor oil and gasoline procurement`
...
14. The Windows **Save As** dialog opens. The default filename
    (e.g. `Standard tier solicitation_<uuid>_<datetime>.docx`) and default folder
    (`C:\temp\assembled_documents\`) are correct — do not change them.
15. Click **Save**.
16. Confirm the Document Assembly status bar reads
    "Saved: Standard tier solicitation_<filename>."
