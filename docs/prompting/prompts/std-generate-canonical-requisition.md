# Generate a Canonical Standard Tier Requisition
...
5. In the Line Item Manager dialog, click the **Document Type** dropdown and select
   **Standard tier requisition [standard]**.
...
15. Choose **Standard tier requisition** from the Document Type list on the left of the form.
16. Wait until the lower-right button is labeled **Generate Document** and the status bar
    reads "Selected: Standard tier requisition".
...
19. In the Fill-in Values dialog, supply only the following two values
    ...
    - **Document Title** = `Motor oil and gasoline procurement`
    - **Requisition Number** = `222287654`
...
21. A Windows **Save As** dialog will open. The default filename (system-generated,
    e.g. `Standard tier requisition_<uuid>_<datetime>.docx`) and default folder
...
23. Confirm the MAKEDOC Dashboard (visible behind or after the Assembly form) shows a
    new Standard tier requisition entry dated today with the title
    "Motor Oil and gasoline proc...".
