# Generate a Canonical Complex Tier Requisition
...
5. In the Line Item Manager dialog, click the **Document Type** dropdown and select
   **Complex tier requisition [complex]**.
...
10. Fill in the second row:
    ...
    - Qty = `1000`
    - Unit Price = `35`
11. Click **Save All**. Confirm the status bar reads "Saved 2 line item(s)." 
...
15. Choose **Complex tier requisition** from the Document Type list on the left of the form.
16. Wait until the lower-right button is labeled **Generate Document** and the status bar
    reads "Selected: Complex tier requisition". The clause list should populate with 7
    clauses.
...
19. In the Fill-in Values dialog, supply only the following two values
    ...
    - **Document Title** = `Paper and Boxes Procurement`
    - **Requisition Number** = `222233333`
...
21. A Windows **Save As** dialog will open. The default filename (system-generated,
    e.g. `Complex tier requisition_<uuid>_<datetime>.docx`) and default folder
