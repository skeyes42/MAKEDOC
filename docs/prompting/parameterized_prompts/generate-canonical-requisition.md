...
17. Choose **{{Tier}} tier requisition** from the Document Type list on the left of the form.
18. Wait until the lower-right button is labeled **Generate Document** and the status bar
    reads "Selected: {{Phase}} tier requisition".
...
23. A Windows **Save As** dialog will open. The default filename (system-generated,
    e.g. `{{Tier}} tier requisition_<uuid>_<datetime>.docx`) and default folder
...
25. Confirm the MAKEDOC Dashboard (visible behind or after the Assembly form) shows a
    new {{Phase}} tier requisition entry dated today with the title
